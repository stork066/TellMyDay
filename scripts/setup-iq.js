...
async function main() {
    ...
    console.log(`→ PUT knowledgesources/${KS_NAME}`);
    {
        ...
        const body = {
            name: KS_NAME,
            kind: "file",
            description: "Synthetic About-Me profile for the Tell My Day AAC demo. Five short markdown files: people, places, activities, food, comfort.",
            fileParameters: {
                ingestionParameters: {
    ...
    const entries = (await readdir(PROFILE_DIR)).filter((n) => n.endsWith(".md") && n !== "README.md");
    if (entries.length === 0) {
